...
def check_fields(ref: Reference, standard: str = "7.0.100") -> list[dict[str, str]]:
    ...
    if not ref.url and ref.medium == "electronic" and ref.type not in ("website", "webpage"):
        ...
        if _is_local_carrier(ref):
            ...
            found.append(_issue("warning", "electronic-no-url",
                                ...
                                "а не ошибка.",
                                "Уточните способ распространения; для сетевого ресурса найдите "
                                "проверенный URL и дату. Не меняйте носитель ради устранения замечания."))
    ...
    if ref.type == "standard":
        found.append(_issue("note", "check-status",
                            ...
                            "сопоставьте статус с целью цитирования; не представляйте историческую редакцию действующей."))
...
def check_string(s: str, standard: str = "7.0.100") -> list[dict[str, str]]:
    ...
    if standard == "7.0.100":
        if "—" not in masked:
            found.append(_issue("note", "no-area-dash",
                                "Не найден разделитель областей. П. 4.6.4 "
                                "ГОСТ Р 7.0.100-2018 разрешает заменять знак «точка и тире» "
                                "точкой, если области выделены шрифтом или начинаются "
                                "с новой строки. Проверка строки не видит шрифты: "
                                "проверьте выполнение этого условия в документе."))
        if not re.search(r"Текст\s*:|Изображение\s*:|Видео\s*:|Аудио\s*:", masked):
            found.append(_issue("note", "no-content-type",
                                ...
                                "Вид содержания и средство доступа — условно-обязательные "
                                "элементы (ГОСТ Р 7.0.100-2018, 5.10.4, 5.10.8). "
                                "По 4.4.2 применимость зависит от задач описания. "
                                "Профиль проекта отключает область по умолчанию; "
                                "уровень note не подтверждает соответствие требованиям организации."))
    ...
    if standard in ("7.0.5", "7.0.108"):
        # Признак области — не тире само по себе, а последовательность
        # «точка, пробел, тире, пробел» (« . — » / « . – »). Одиночное тире
        # внутри слова или между буквами — часть выходных данных
        # («V–A–C press», «Springer–Verlag», архивный шифр «Ф–1»,
        # двойные фамилии), а не разделитель областей.
